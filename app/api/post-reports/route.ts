/**
 * File purpose
 * - Create an "open" report for a whole post by an authenticated user.
 * Main responsibilities
 * - Validate reason and optional details, then insert into public.post_reports.
...
import { POST_REPORT_REASONS } from "@/lib/api/postReports";

type Reason = (typeof POST_REPORT_REASONS)[number];

function isReason(value: unknown): value is Reason {
  return POST_REPORT_REASONS.includes(value as Reason);
}
...
export async function POST(req: Request) {
  ...
  if (!Number.isFinite(postId) || postId <= 0) {
    return NextResponse.json({ error: "bad_request_postId" }, { status: 400 });
  }
  ...
  const { error: insErr } = await supabase.from("post_reports").insert({
    post_id: postId,
    reason,
