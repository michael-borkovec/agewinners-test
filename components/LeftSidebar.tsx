...
import HotMessagePanel from "@/components/HotMessagePanel";
...
const PROFILE_LINKS = [
  ...
  { href: "/profile/social", label: "Kontakt" },
...
const STATS_LINKS = [
  ...
  { href: "/stats?section=posts", section: "posts", label: "Vývoj příspěvků" },
  ...
  { href: "/stats?section=aw-invites", section: "aw-invites", label: "AW pozvánky" },
...
function isProfileMenuRoute(pathname: string | null | undefined) {
...
export default function LeftSidebar({ loadingProfile, profile }: LeftSidebarProps) {
  ...
  const showProfileMenu = isProfileMenuRoute(pathname);
  ...
  return (
    <aside className="w-full shrink-0 rounded-2xl bg-white px-4 py-5">
      ...
      {showStatsMenu ? (
        <div className="mt-6 rounded-2xl bg-white p-4">
          <div className="text-sm font-semibold text-gray-900">Můj vývoj</div>
            ...
            {STATS_LINKS.map((item) => {
              ...
              return (
                <Link
                  ...
                  className={[
                    "block rounded-xl px-3 py-2 text-sm font-semibold transition",
                    active ? "bg-[#effdef] text-emerald-800" : "text-gray-700 hover:bg-gray-50",
                  ].join(" ")}
                >
                  {item.label}
                </Link>
            ...
            {NETWORK_LINKS.map((item) => {
              ...
              return (
                ...
                >
                  ...
                  {badgeCount > 0 ? (
                    <span className="inline-flex min-w-[20px] items-center justify-center rounded-full bg-[#32CD32] px-1.5 py-0.5 text-[11px] font-bold text-white">
